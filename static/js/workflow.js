...
function waitForDependencies() {
    return new Promise((resolve) => {
        const checkDeps = () => {
            if (window.apiCall && window.state) {
                resolve();
            } else {
...
async function loadPipelines() {
    try {
        const data = await safeApiCall('/api/pipelines');
    ...
    listEl.innerHTML = workflowState.pipelines.map(pipeline => `
        <div style="background: var(--bg-secondary); padding: var(--spacing-md); border-radius: var(--border-radius); border-left: 4px solid ${pipeline.enabled ? 'var(--success-color)' : 'var(--text-muted)'};">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: var(--spacing-sm);">
                <div style="flex: 1;">
                    <h4 style="margin: 0; margin-bottom: var(--spacing-xs);">${escapeHtml(pipeline.name)}</h4>
                    <p style="margin: 0; color: var(--text-muted); font-size: 0.875rem;">${escapeHtml(pipeline.description || 'No description')}</p>
                </div>
...
async function runPipeline(pipelineId) {
    ...
    try {
        const data = await safeApiCall(`/api/pipelines/${pipelineId}/execute`, {
            method: 'POST',
...
async function deletePipeline(pipelineId) {
    ...
    try {
        const data = await safeApiCall(`/api/pipelines/${pipelineId}`, {
            method: 'DELETE'
        });
...
async function savePipeline(event) {
    ...
    try {
        ...
        if (workflowState.editingPipeline) {
            // Update existing
            data = await safeApiCall(`/api/pipelines/${workflowState.editingPipeline.id}`, {
                method: 'PUT',
            ...
            // Create new
            data = await safeApiCall('/api/pipelines', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(pipelineData)
            });
...
async function loadAvailableActions() {
    try {
        const data = await safeApiCall('/api/pipelines/actions');
...
async function loadTemplates() {
    try {
        const data = await safeApiCall('/api/pipelines/templates');
...
function renderTemplates() {
    ...
    if (workflowState.templates.length === 0) {
        listEl.innerHTML = '<p style="color: var(--text-muted); text-align: center; padding: var(--spacing-md);">No templates available.</p>';
        return;
    }
    ...
    listEl.innerHTML = workflowState.templates.map(template => `
        <div style="background: var(--bg-secondary); padding: var(--spacing-md); border-radius: var(--border-radius);">
            <h4 style="margin: 0; margin-bottom: var(--spacing-sm);">${escapeHtml(template.name)}</h4>
            <p style="margin: 0; margin-bottom: var(--spacing-sm); color: var(--text-muted); font-size: 0.875rem;">${escapeHtml(template.description)}</p>
            <div style="margin-bottom: var(--spacing-sm); font-size: 0.875rem;">
                ...
                <ul style="margin: var(--spacing-xs) 0; padding-left: var(--spacing-lg);">
                    ${template.actions.map(a => `<li>${escapeHtml(getActionLabel(a.type))}</li>`).join('')}
                </ul>
            </div>
...
async function useTemplate(templateId) {
    try {
        const data = await safeApiCall(`/api/pipelines/templates/${templateId}`, {
            method: 'POST'
        });
...
async function loadExecutionLogs() {
    try {
        const data = await safeApiCall('/api/executions/recent?limit=50');
        ...
        return `
            <div style="background: var(--bg-secondary); padding: var(--spacing-sm); border-radius: var(--border-radius); border-left: 4px solid ${statusColor};">
                <div style="display: flex; justify-content: space-between; font-size: 0.875rem;">
                    <div>
                        <strong>${escapeHtml(exec.pipeline_name)}</strong>
...
waitForDependencies().then(() => {
    console.log('Workflow module: Dependencies loaded');

    // Workflows modal
    const workflowsBtn = document.getElementById('workflowsBtn');
    const workflowsClose = document.getElementById('workflowsClose');
    const workflowsOverlay = document.getElementById('workflowsOverlay');

    if (workflowsBtn) workflowsBtn.addEventListener('click', openWorkflowsModal);
    if (workflowsClose) workflowsClose.addEventListener('click', closeWorkflowsModal);
    if (workflowsOverlay) workflowsOverlay.addEventListener('click', closeWorkflowsModal);
    ...
    const pipelineBuilderClose = document.getElementById('pipelineBuilderClose');
    const pipelineBuilderOverlay = document.getElementById('pipelineBuilderOverlay');
    const cancelPipelineBtn = document.getElementById('cancelPipelineBtn');
    const pipelineBuilderForm = document.getElementById('pipelineBuilderForm');

    if (createPipelineBtn) createPipelineBtn.addEventListener('click', openPipelineBuilder);
    if (pipelineBuilderClose) pipelineBuilderClose.addEventListener('click', closePipelineBuilder);
    if (pipelineBuilderOverlay) pipelineBuilderOverlay.addEventListener('click', closePipelineBuilder);
    if (cancelPipelineBtn) cancelPipelineBtn.addEventListener('click', closePipelineBuilder);
    if (pipelineBuilderForm) pipelineBuilderForm.addEventListener('submit', savePipeline);

    // Action selector
    const addActionBtn = document.getElementById('addActionBtn');
    const actionSelectorClose = document.getElementById('actionSelectorClose');
    const actionSelectorOverlay = document.getElementById('actionSelectorOverlay');

    if (addActionBtn) addActionBtn.addEventListener('click', openActionSelector);
    if (actionSelectorClose) actionSelectorClose.addEventListener('click', closeActionSelector);
    if (actionSelectorOverlay) actionSelectorOverlay.addEventListener('click', closeActionSelector);

    console.log('Workflow Automation module loaded and initialized');
});
